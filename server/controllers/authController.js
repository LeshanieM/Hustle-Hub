...
const registerUser = async (req, res) => {
    try {
        ...
        if (!studentId) {
            return res.status(400).json({ message: 'Could not detect a valid Student ID (e.g., IT24375659) from the image' });
        }
...
const verifyOTP = async (req, res) => {
    try {
        ...
        if (!tempUser) {
            return res.status(404).json({ message: 'No registration pending or OTP has timed out (10 minutes expired)' });
        }
        ...
        res.status(200).json({ message: 'Account successfully registered and verified! You can now login.' });
