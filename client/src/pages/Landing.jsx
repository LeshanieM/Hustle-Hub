...
export default function Landing() {
  ...
  useEffect(() => {
    ...
    // Fetch 3 most recent stores
    axios
      .get(`${API_URL}/stores`)
      .then((res) => {
        const all = res.data?.stores || res.data || [];
        // Only show ACTIVE or PENDING stores, take latest 3
        const latest = all.filter((s) => s.status !== 'SUSPENDED').slice(0, 3);
        setStores(latest);
      })
    ...
    <div className="bg-[#f6f6f8] text-slate-900 antialiased font-display min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              },
              {
                name: 'Food',
                icon: 'restaurant',
                img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBXMk3n8Cn0Ae3xvskSC0XfcF-EpJqsDioiDJ84H-G6JMG9XiMpLgBwRXWSE3FFg9S3RV9_vbUF3XcA51PCysYBbtLgzyhazpqnVtqt4B1iR3jV37dqrY6kI9bI6BbBvkR5LCp86siiBY8iwzcsTgutY2CymEEv2HKZSZAE5zDtBjXfPErOAbkeSl_Xfrtg8qB49eG7IAHIB3phDsr0rE9vJYUTIKyHmFSGX2QzX3Kny93UtvI5dxlzrvspEPkQGfO8gQHiTryTvw',
              },
              {
                name: 'Services',
              ...
              <Link
                key={cat.name}
                className="group relative overflow-hidden rounded-2xl aspect-[4/3] bg-slate-200 no-underline shadow-sm"
                to="#"
              >
        ...
        <section className="mb-16">
          <div className="mb-8">
            ...
            <p className="text-slate-500 font-medium mt-1 text-sm">
              Support your fellow students and discover fresh hustle.
            </p>
          </div>
          {stores.length > 0 ? (
              ...
              {stores.map((store) => {
                ...
                return (
                  <Link
                    ...
                    className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 no-underline text-inherit"
                  >
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-16 h-16 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden border border-slate-100">
                        {store.logoUrl ? (
                          ...
                          <span className="text-2xl font-black text-[#1111d4]">
                      ...
                      <div>
                        ...
                        <span
                          className={`inline-block px-2 py-0.5 mt-1 ${statusTag.color} text-[10px] font-bold uppercase tracking-wider rounded`}
                        >
                          {statusTag.label}
                        </span>
                      </div>
                    </div>
                    <p className="text-slate-500 font-medium mb-6 text-sm leading-relaxed line-clamp-2">
                      {store.description || 'Check out this student-run store!'}
                    </p>
                    <span className="block w-full py-2.5 rounded-lg text-[#1111d4] font-bold text-center hover:bg-[#1111d4] hover:text-white transition-all">
                      Visit Store
                    </span>
                  </Link>
                );
            ...
            <div className="text-center py-16 text-slate-400">
              ...
              <p className="text-sm font-bold text-slate-500">No stores yet</p>
              <p className="text-xs mt-1">
                Be the first to open a store on campus!
              </p>
            </div>
          )}
        </section>
                ...
                >
                  <div className="relative rounded-3xl overflow-hidden aspect-square bg-slate-100 mb-4 shadow-sm group-hover:shadow-lg transition-all duration-500">
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 w-10 h-10 bg-white/95 backdrop-blur rounded-xl flex items-center justify-center text-slate-300 hover:text-red-500 transition-all shadow-md active:scale-90 border-none cursor-pointer"
                    >
                      <MaterialIcon name="favorite" size={20} />
                    </button>
                  </div>
                  <div className="space-y-1.5 px-0.5">
                    <p className="text-[10px] font-bold text-[#1111d4] uppercase tracking-widest">
                      {prod.type || 'General'}
                    </p>
                    ...
                    <div className="flex items-center justify-between pt-1">
                      ...
                      <span className="w-10 h-10 bg-slate-900 text-white rounded-xl flex items-center justify-center group-hover:bg-[#1111d4] transition-all shadow-md">
                        <MaterialIcon name="add_shopping_cart" size={20} />
                      </span>
                    </div>
                  </div>
                </Link>
        ...
        <section className="bg-[#1111d4] rounded-[32px] p-8 md:p-16 text-center text-white relative overflow-hidden shadow-2xl shadow-[#1111d4]/20">
          <div
            className="absolute inset-0 bg-white/10 opacity-10 pointer-events-none"
            style={{
              backgroundImage:
          ...
          <div className="relative z-10">
            <h2 className="text-3xl md:text-5xl font-black mb-6 leading-tight tracking-tight">
              Ready to start your own hustle?
            </h2>
            <p className="text-lg md:text-xl text-white/80 mb-10 max-w-2xl mx-auto font-medium">
              Join thousands of students earning extra cash by selling
