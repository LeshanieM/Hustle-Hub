      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <section className="mb-16">
          ...
          {stores.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {stores.map((store) => {
                const statusTag = store.status === 'ACTIVE'
                  ...
                  <Link key={store._id} to={`/store/${encodeURIComponent(store.storeName)}`} className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg transition-all duration-300 no-underline text-inherit">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="w-16 h-16 rounded-xl bg-slate-50 flex items-center justify-center overflow-hidden border border-slate-100">
                        {store.logoUrl ? (
                          <img alt={store.storeName} className="w-full h-full object-cover" src={store.logoUrl} />
                        ) : (
                          <span className="text-2xl font-black text-[#1111d4]">{store.storeName?.charAt(0)}</span>
                        )}
                      </div>
                      <div>
                        <h3 className="font-bold text-lg text-slate-900 leading-tight">{store.storeName}</h3>
                        <span className={`inline-block px-2 py-0.5 mt-1 ${statusTag.color} text-[10px] font-bold uppercase tracking-wider rounded`}>{statusTag.label}</span>
                      </div>
                    </div>
            ...
            <div className="text-center py-16 text-slate-400">
              <span className="material-symbols-outlined text-5xl mb-3 opacity-40">storefront</span>
              <p className="text-sm font-bold text-slate-500">No stores yet</p>
              <p className="text-xs mt-1">Be the first to open a store on campus!</p>
            </div>
          )}
        </section>
        ...
        <section className="mb-16">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link className="text-[#1111d4] font-bold text-sm hover:underline no-underline" to="/customer/products">View All</Link>
          </div>
          {products.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {products.map((prod) => (
                <Link key={prod._id} to={`/customer/products/${prod._id}`} className="group cursor-pointer no-underline text-inherit">
                  <div className="relative rounded-3xl overflow-hidden aspect-square bg-slate-100 mb-4 shadow-sm group-hover:shadow-lg transition-all duration-500">
                    {prod.imageUrl ? (
                      <img className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" src={prod.imageUrl} alt={prod.name} />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center bg-slate-100">
                        <span className="material-symbols-outlined text-6xl text-slate-300">image</span>
                      </div>
                    )}
                    <button onClick={(e) => e.preventDefault()} className="absolute top-3 right-3 w-10 h-10 bg-white/95 backdrop-blur rounded-xl flex items-center justify-center text-slate-300 hover:text-red-500 transition-all shadow-md active:scale-90 border-none cursor-pointer">
                  ...
                  <div className="space-y-1.5 px-0.5">
                    ...
                    <div className="flex items-center justify-between pt-1">
                      <span className="text-lg font-black text-slate-900">${Number(prod.price).toFixed(2)}</span>
            ...
            <div className="text-center py-16 text-slate-400">
              <span className="material-symbols-outlined text-5xl mb-3 opacity-40">shopping_bag</span>
              <p className="text-sm font-bold text-slate-500">No products yet</p>
              <p className="text-xs mt-1">Products will appear here as sellers add them</p>
            </div>
          )}
        </section>
